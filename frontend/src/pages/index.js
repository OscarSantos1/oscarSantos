      ...
      <main className='bg-[#FAFAFC] h-screen w-screen py-10 pl-10 lg:pr-10 md:pr-5'>
        ...
        <div className='flex justify-between items-center h-1/3 min-h-[226px] overflow-clip lg:pl-14 lg:pr-10'>
          <div className='h-full flex items-center w-1/2'>
            <p className='lg:text-[16px] md:text-[14px] pr-4 md:pr-10 '>I'm passionate about creating new products and tools. I love working with other people and most importantly helping a vision come to life.</p>
          </div>
        ...
        <div className='h-1/2'>
          <div className='hidden md:flex md:flex-col md:items-center md:justify-center bg-[#2954B5] h-full w-full min-w-[840px] ml-[-40px] mt-10 rounded-r-full'>
            <h3 className='text-lg text-[#E7E7E7] -mt-4 mb-4 fade-in' >{oneSelected ? selected.name : 'Latest Work'}</h3>
            <div className={`px-14 flex ${oneSelected ? 'justify-start' : 'justify-between'} w-full`}>
              {projects.map((project) => <ProjectCard key={project.id} title={project.name} color={project.brandColor} display={!oneSelected || selected === project} set={() => {setOneSelected(true); setSelected(project)}} />)}
            </div>
          </div>
        </div>
        <div>
